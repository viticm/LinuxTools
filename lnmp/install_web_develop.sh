...
{
    cd ${cInstallFile}
    local cNginxPackage=tengine-1.4.6.tar.gz
    local cNginxPackageDir=tengine-1.4.6
    echo
    echo "***************************************"
...
function InstallPHP()
{
    ...
    if [[ -z ${cPhpVersion} ]]; then
        ...
        cPhpVersion="5.3"
        cPhpPackage="php-5.3.14.tar.bz2"
        cPhpPackageDir="php-5.3.14"
    else
...
#PHP-FPM
    ...
    if [[ -f ${cPhpFpmConfFile} ]] ; then
        ...
        sed "201 c\pm.max_children = ${pm_max_children}" -i ${cInstallPhpPath}etc/php-fpm.conf
        sed "206 c\pm.start_servers = ${pm_start_servers}" -i ${cInstallPhpPath}etc/php-fpm.conf
        sed "211 c\pm.min_spare_servers = ${pm_min_spare_servers}" -i ${cInstallPhpPath}etc/php-fpm.conf
        sed "216 c\pm.max_spare_servers = ${pm_max_spare_servers}" -i ${cInstallPhpPath}etc/php-fpm.conf
        sed "227 c\pm.max_requests = ${pm_max_requests}" -i ${cInstallPhpPath}etc/php-fpm.conf
        sed "132 c\user = ${pm_user}" -i ${cInstallPhpPath}etc/php-fpm.conf
        sed "133 c\group = ${pm_group}" -i ${cInstallPhpPath}etc/php-fpm.conf
    fi
